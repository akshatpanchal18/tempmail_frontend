...
function InBoxA({id}) {
  ...
  useEffect(() => {
    if (isData && isData.messages) {
      const sorted = [...isData.messages].sort((a, b) => 
  ...
  const getInboxdetails = async () => {
    try {
      const response = await fetch(`http://localhost:4000/api/v1/inbox/inbox-data/mail/inboxes`, {
        method: "POST",
    ...
    <Wrapper>
      <div className="inbox">
        ...
        <div className="inbox-list">
          <p style={{textAlign:"center",color:"GrayText"}}>if mail not reflect in inbox use refresh button</p>
          {emailCount > 0 ? (
             inbox && inbox.map((message) => (
              <div key={message._id} className="inbox-card">
                ...
                <div className="inbox-card-body">
                  ...
                  <p className="message-preview">Preview: {message.text.slice(0, 50)}...</p>
                  ...
                  <div className="email-details">
                    <div className="email-header">
                      ...
                      <div className="close-div">
                        <span><Timestamp timestamp={message.receivedAt} /></span>
